...
def apply_memit_to_model(
    ...
    config_to_log = {
        ...
        "layer_module_tmp": getattr(hparams, "layer_module_tmp", None),
        "fact_token": getattr(hparams, "fact_token", None),
        "clamp_norm_factor": getattr(hparams, "clamp_norm_factor", None),
        ...
        "mom2_update_weight": getattr(hparams, "mom2_update_weight", None),
        "beta_hse": beta_hse,
        "alpha": alpha,
        "dataset": dataset,
    ...
    try:
        config_to_log["n_requests"] = len(requests)
    except Exception:
        pass
    try:
        config_to_log["device"] = str(next(model.parameters()).device)
    except Exception:
        pass
    print("[MEMIT] Running apply_memit_to_model with config:")
    print(json.dumps(config_to_log, ensure_ascii=False, indent=2))
    ...
    if save_weights:
        save_path = f'./Edited_Weight/MEMIT/{hparams.model_name.split("/")[-1]}/{dataset}_weight_data_batch_{batch_size}_{beta_hse}_{alpha}/'
...
def project_B_into_soft_dirs(A, B, energy_ratio=0.8, alpha=0.8):
    print(f"Projection Start with energy_ratio (beta): {energy_ratio} and alpha: {alpha}")
