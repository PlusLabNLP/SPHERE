...
def apply_memit_rect_to_model(
    ...
    if save_weights:
        save_path = f'./Edited_Weight/MEMIT_rect/{hparams.model_name.split("/")[-1]}/{dataset}_weight_data_batch_{batch_size}_{beta_hse}_{alpha}/'
    ...
    with torch.no_grad():
        for w_name, (key_mat, val_mat) in deltas.items():
            ...
            upd_matrix_rect = torch.zeros_like(upd_matrix)
            upd_matrix_rect[mask] = upd_matrix[mask]

            if beta_hse > 0:
                upd_matrix_proj, P_soft, U = project_B_into_soft_dirs(w, upd_matrix_rect.float(), energy_ratio=beta_hse, alpha=alpha)
                upd_matrix_final = upd_matrix_proj.float()
            else:
                upd_matrix_final = upd_matrix_rect.float()
