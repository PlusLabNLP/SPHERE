...
def apply_AlphaEdit_to_model(
    ...
    if save_weights:
        save_path = f'./Edited_Weight/AlphaEdit/{hparams.model_name.split("/")[-1]}/{dataset}_weight_data_batch_{batch_size}_{beta_hse}_{alpha}/'
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        with torch.no_grad():
            ...
            if beta_hse > 0:
                upd_matrix_proj, P_soft, U = project_B_into_soft_dirs(w, upd_matrix, energy_ratio=beta_hse, alpha=alpha)
                w[...] += upd_matrix_proj.float()
            else:
                w[...] += upd_matrix

            if save_weights:
                modified_weights[weight_name] = w.detach().clone()
